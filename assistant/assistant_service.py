...
def __get_transcoded_audio_file_path(data:bytes) -> str:
    ...
    local_output_file_path = create_unique_tmp_file(file_suffix='transcoded_user_audio_mp3')
...
async def handle_audio_from_user(file:bytes) -> str:
    ...
    transcript_content_text = convert_audio_to_text(transconded_user_audio_file_path)
    text_content = transcript_content_text['text']
    ai_text_reply = invoke_app(text_content)
